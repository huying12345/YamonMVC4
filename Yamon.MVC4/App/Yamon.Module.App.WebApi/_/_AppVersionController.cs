...
//     如果想修改此文件的方法，请在此文件的上级目录中的AppVersionController重写(override)该方法。
...
namespace Yamon.Module.App.WebApi
{
    ...
    /// 版本模型
    /// </summary>
    public partial class _AppVersionController :BaseController
    {

        public AppVersionDAL dal = new AppVersionDAL();
        ...
        [ValidateInput(false)]
        [LogFilter]
        public virtual ActionResult Create(AppVersion model,bool returnData=false)
        {
            object newId = "";
            try
            {
                ...
                if (result > 0)
                {
                    ...
                    hash["id"] = newId;
                    if(returnData){
                        AppVersion newrow = dal.GetModelByID(newId);
                        hash["data"] = newrow;
                    }
                }
        ...
        [LogFilter]
        public virtual ActionResult Edit(AppVersion model,bool returnData=false)
        {
            try
            {
               ...
               if (result > 0)
                {
                    ...
                    if(returnData){
                        AppVersion newrow = dal.GetModelByID(model.VersionID);
                        hash["data"] = newrow;
                    }
                }
            }
        ...
        [NonAction]        
        public  virtual List<AppVersion> GetGridList(string filterId, int showValue, out int totalRows)
        {
            totalRows = 0;
            ...
            List<AppVersion> modelList = new List<AppVersion>();
            switch(filterId)
            {
    ...
    public virtual ActionResult Delete(string id)
    {
        ...
        [CheckPurview(pageCode:"App_AppVersion_Show")]        
        public ActionResult GetEntityByID(string id)
        {
            AppVersion model = dal.GetModelByID(id);
            if (model != null)
            {
                hash["data"] = model;
                hash["success"] = true;

...
//     如果想修改此文件的方法，请在此文件的上级目录中的AppLogController重写(override)该方法。
...
namespace Yamon.Module.App.Web.Controllers
{
    ...
    /// 日志模型
    /// </summary>
    public partial class _AppLogController :BaseController
    {

        public AppLogDAL dal = new AppLogDAL();
        ...
        public ActionResult Index(string m,string filterId)
        {
            ...
            string formPath = string.Format("~/Areas/App/Views/AppLog/_/{0}.cshtml", m);
...
public virtual ActionResult Create(string id = "")
{
    ViewBag.ID = id;
    ViewBag.DAL=dal;
    AppLog model= dal.GetCreateFormDefaultValue();
    model=dal.GetModelShowValue(model);
    string formPath = SiteCommon.GetCustomPage("~/Areas/App/Views/AppLog/_/Create.cshtml");
...
public virtual ActionResult Show(string id = "")
{
    ViewBag.ID = id;
    ViewBag.DAL=dal;
    AppLog model = dal.GetModelByID(id);
    model=dal.GetModelShowValue(model);
    string formPath = SiteCommon.GetCustomPage("~/Areas/App/Views/AppLog/_/Show.cshtml");
    return View(formPath,model);
}

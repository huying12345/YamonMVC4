...
//     如果想修改此文件的方法，请在此文件的上级目录中的UserConfigController重写(override)该方法。
...
namespace Yamon.Module.UCenter.WebApi
{
    ...
    /// 用户配置
    /// </summary>
    public partial class _UserConfigController :BaseController
    {

        public UserConfigDAL dal = new UserConfigDAL();
        ...
        [ValidateInput(false)]
        [LogFilter]
        public virtual ActionResult Create(UserConfig model,bool returnData=false)
        {
            object newId = "";
            try
            {
                ...
                if (result > 0)
                {
                    ...
                    hash["id"] = newId;
                    if(returnData){
                        UserConfig newrow = dal.GetModelByID(newId);
                        hash["data"] = newrow;
                    }
                }
        ...
        [LogFilter]
        public virtual ActionResult Edit(UserConfig model,bool returnData=false)
        {
            try
            {
               ...
               if (result > 0)
                {
                    hash["id"] = model.ID;
                    if(returnData){
                        UserConfig newrow = dal.GetModelByID(model.ID);
                        hash["data"] = newrow;
                    }
                }
            }
        ...
        [NonAction]        
        public  virtual List<UserConfig> GetGridList(string filterId, int showValue, out int totalRows)
        {
            totalRows = 0;
            ...
            List<UserConfig> modelList = new List<UserConfig>();
            switch(filterId)
            {
    ...
    public virtual ActionResult Delete(string id)
    {
        ...
        [CheckPurview(pageCode:"UCenter_UserConfig_Show")]        
        public ActionResult GetEntityByID(string id)
        {
            UserConfig model = dal.GetModelByID(id);
            if (model != null)
            {
                hash["data"] = model;
                hash["success"] = true;

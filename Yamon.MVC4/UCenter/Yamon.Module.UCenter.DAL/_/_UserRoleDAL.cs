...
//     如果想修改此文件的方法，请在此文件的上级目录中的UserRoleDAL重写(override)该方法。
...
namespace Yamon.Module.UCenter.DAL
{
    ...
    /// 用户角色实体类
    ///</summary>
    public partial class _UserRoleDAL : BaseModelDAL<UserRole>
    {
        public _UserRoleDAL():base("UCenter")
        {
        }
		...
		public virtual List<UserRole> GetEntityListByPage(out int totalRows,int topN = 0, int page=1,int rows=20,string orderBy="")
		{
			string order = "";
			...
			string fields="UserRoleID,UserID,RoleID,IsDefault";
	    ...
	    public virtual UserRole GetModelValue(UserRole model)
        {
            return model;
        }
		/// <summary>
        /// 设置用户角色实体(UserRole)的显示值
        /// </summary>
        /// <param name="model">用户角色实体(UserRole)</param>
        /// <returns>用户角色实体(UserRole)</returns>
		public virtual UserRole GetModelShowValue(UserRole model,bool clearValue=false)
        {
		...
		/// <summary>
        /// 设置用户角色实体(UserRole)的列表显示值
        /// </summary>
        /// <param name="model">用户角色实体(UserRole)</param>
        /// <returns>用户角色实体(UserRole)</returns>
		public virtual UserRole GetModelGridShowValue(UserRole model){
			model=GetModelShowValue(model);
			return model;
        }

        public virtual UserRole GetInfoByID(object id){
            UserRole model = GetModelByID(id);
            model = GetModelShowValue(model);
            return model;
        } 

...
//     如果想修改此文件的方法，请在此文件的上级目录中的MemberGradeController重写(override)该方法。
...
namespace Yamon.Module.Member.WebApi
{
    ...
    /// 会员等级
    /// </summary>
    public partial class _MemberGradeController :BaseController
    {

        public MemberGradeDAL dal = new MemberGradeDAL();
        ...
        [ValidateInput(false)]
        [LogFilter]
        public virtual ActionResult Create(MemberGrade model,bool returnData=false)
        {
            object newId = "";
            try
            {
                ...
                if (result > 0)
                {
                    ...
                    hash["id"] = newId;
                    if(returnData){
                        MemberGrade newrow = dal.GetModelByID(newId);
                        hash["data"] = newrow;
                    }
                }
        ...
        [LogFilter]
        public virtual ActionResult Edit(MemberGrade model,bool returnData=false)
        {
            try
            {
               ...
               if (result > 0)
                {
                    hash["id"] = model.MemberGradeId;
                    if(returnData){
                        MemberGrade newrow = dal.GetModelByID(model.MemberGradeId);
                        hash["data"] = newrow;
                    }
                }
            }
        ...
        [NonAction]        
        public  virtual List<MemberGrade> GetGridList(string filterId, int showValue, out int totalRows)
        {
            totalRows = 0;
            ...
            List<MemberGrade> modelList = new List<MemberGrade>();
            switch(filterId)
            {
        ...
        public  virtual ActionResult GridByFilterID(string filterId)
        {
            ...
            catch (Exception ex)
            {
                hash["success"] = false;
                hash["message"] = ex.Message;
                hash["rows"] = new List<MemberGrade>();
            }
            return Content(JsonConvert.SerializeObject(hash)); 
        }
    ...
    public virtual ActionResult Delete(string id)
    {
        ...
        [CheckPurview(pageCode:"Member_MemberGrade_Show")]        
        public ActionResult GetEntityByID(string id)
        {
            MemberGrade model = dal.GetModelByID(id);
            if (model != null)
            {
                hash["data"] = model;
                hash["success"] = true;

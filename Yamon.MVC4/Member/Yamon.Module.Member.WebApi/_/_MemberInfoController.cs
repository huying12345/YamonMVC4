...
//     如果想修改此文件的方法，请在此文件的上级目录中的MemberInfoController重写(override)该方法。
...
namespace Yamon.Module.Member.WebApi
{
    ...
    /// 会员
    /// </summary>
    public partial class _MemberInfoController :BaseController
    {

        public MemberInfoDAL dal = new MemberInfoDAL();
        ...
        [ValidateInput(false)]
        [LogFilter]
        public virtual ActionResult Create(MemberInfo model,bool returnData=false)
        {
            object newId = "";
            try
            {
                ...
                if (result > 0)
                {
                    ...
                    hash["id"] = newId;
                    if(returnData){
                        MemberInfo newrow = dal.GetModelByID(newId);
                        hash["data"] = newrow;
                    }
                }
        ...
        [LogFilter]
        public virtual ActionResult Edit(MemberInfo model,bool returnData=false)
        {
            try
            {
               ...
               if (result > 0)
                {
                    hash["id"] = model.MemberInfoId;
                    if(returnData){
                        MemberInfo newrow = dal.GetModelByID(model.MemberInfoId);
                        hash["data"] = newrow;
                    }
                }
            }
        ...
        [NonAction]        
        public  virtual List<MemberInfo> GetGridList(string filterId, int showValue, out int totalRows)
        {
            totalRows = 0;
            ...
            List<MemberInfo> modelList = new List<MemberInfo>();
            switch(filterId)
            {
    ...
    public virtual ActionResult Delete(string id)
    {
        ...
        [CheckPurview(pageCode:"Member_MemberInfo_Show")]        
        public ActionResult GetEntityByID(string id)
        {
            MemberInfo model = dal.GetModelByID(id);
            if (model != null)
            {
                hash["data"] = model;
                hash["success"] = true;

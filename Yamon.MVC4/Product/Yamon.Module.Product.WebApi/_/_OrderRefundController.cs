...
//     如果想修改此文件的方法，请在此文件的上级目录中的OrderRefundController重写(override)该方法。
...
namespace Yamon.Module.Product.WebApi
{
    ...
    /// 订单退票
    /// </summary>
    public partial class _OrderRefundController :BaseController
    {

        public OrderRefundDAL dal = new OrderRefundDAL();
        ...
        [ValidateInput(false)]
        [LogFilter]
        public virtual ActionResult Create(OrderRefund model,bool returnData=false)
        {
            object newId = "";
            try
            {
                ...
                if (result > 0)
                {
                            newId = model.RefundID;

                    hash["id"] = newId;
                    if(returnData){
                        OrderRefund newrow = dal.GetModelByID(newId);
                        hash["data"] = newrow;
                    }
                }
        ...
        [LogFilter]
        public virtual ActionResult Edit(OrderRefund model,bool returnData=false)
        {
            try
            {
               ...
               if (result > 0)
                {
                    hash["id"] = model.RefundID;
                    if(returnData){
                        OrderRefund newrow = dal.GetModelByID(model.RefundID);
                        hash["data"] = newrow;
                    }
                }
            }
        ...
        [NonAction]        
        public  virtual List<OrderRefund> GetGridList(string filterId, int showValue, out int totalRows)
        {
            totalRows = 0;
            ...
            List<OrderRefund> modelList = new List<OrderRefund>();
            switch(filterId)
            {
                    case "OrderRefund_Models":
                    {
        ...
        public  virtual ActionResult GridByFilterID(string filterId)
        {
            ...
            catch (Exception ex)
            {
                hash["success"] = false;
                hash["message"] = ex.Message;
                hash["rows"] = new List<OrderRefund>();
            }
            return Content(JsonConvert.SerializeObject(hash)); 
        }
    ...
    public virtual ActionResult Delete(string id)
    {
        ...
        [CheckPurview(pageCode:"Product_OrderRefund_Show")]        
        public ActionResult GetEntityByID(string id)
        {
            OrderRefund model = dal.GetModelByID(id);
            if (model != null)
            {
                hash["data"] = model;
                hash["success"] = true;

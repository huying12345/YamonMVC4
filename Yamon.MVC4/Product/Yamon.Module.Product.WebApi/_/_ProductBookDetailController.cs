...
//     如果想修改此文件的方法，请在此文件的上级目录中的ProductBookDetailController重写(override)该方法。
...
namespace Yamon.Module.Product.WebApi
{
    ...
    /// 预约明细
    /// </summary>
    public partial class _ProductBookDetailController :BaseController
    {

        public ProductBookDetailDAL dal = new ProductBookDetailDAL();
        ...
        [ValidateInput(false)]
        [LogFilter]
        public virtual ActionResult Create(ProductBookDetail model,bool returnData=false)
        {
            object newId = "";
            try
            {
                ...
                if (result > 0)
                {
                    ...
                    hash["id"] = newId;
                    if(returnData){
                        ProductBookDetail newrow = dal.GetModelByID(newId);
                        hash["data"] = newrow;
                    }
                }
        ...
        [LogFilter]
        public virtual ActionResult Edit(ProductBookDetail model,bool returnData=false)
        {
            try
            {
               ...
               if (result > 0)
                {
                    ...
                    if(returnData){
                        ProductBookDetail newrow = dal.GetModelByID(model.ProductBookDetailID);
                        hash["data"] = newrow;
                    }
                }
            }
        ...
        [NonAction]        
        public  virtual List<ProductBookDetail> GetGridList(string filterId, int showValue, out int totalRows)
        {
            totalRows = 0;
            ...
            List<ProductBookDetail> modelList = new List<ProductBookDetail>();
            switch(filterId)
            {
        ...
        public  virtual ActionResult GridByFilterID(string filterId)
        {
            ...
            catch (Exception ex)
            {
                hash["success"] = false;
                hash["message"] = ex.Message;
                hash["rows"] = new List<ProductBookDetail>();
            }
            return Content(JsonConvert.SerializeObject(hash)); 
        }
    ...
    public virtual ActionResult Delete(string id)
    {
        ...
        [CheckPurview(pageCode:"Product_ProductBookDetail_Show")]        
        public ActionResult GetEntityByID(string id)
        {
            ProductBookDetail model = dal.GetModelByID(id);
            if (model != null)
            {
                hash["data"] = model;
                hash["success"] = true;

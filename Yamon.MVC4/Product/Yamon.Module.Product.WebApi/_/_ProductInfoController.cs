...
//     如果想修改此文件的方法，请在此文件的上级目录中的ProductInfoController重写(override)该方法。
...
namespace Yamon.Module.Product.WebApi
{
    ...
    /// 商品
    /// </summary>
    public partial class _ProductInfoController :BaseController
    {

        public ProductInfoDAL dal = new ProductInfoDAL();

        public virtual ActionResult Grid1_Models()
        {
             return GridByFilterID("ProductInfo_Models");
        } 
        ...
        [ValidateInput(false)]
        [LogFilter]
        public virtual ActionResult Create(ProductInfo model,bool returnData=false)
        {
            object newId = "";
            try
            {
                ...
                if (result > 0)
                {
                    ...
                    hash["id"] = newId;
                    if(returnData){
                        ProductInfo newrow = dal.GetModelByID(newId);
                        hash["data"] = newrow;
                    }
                }
        ...
        [LogFilter]
        public virtual ActionResult Edit(ProductInfo model,bool returnData=false)
        {
            try
            {
               ...
               if (result > 0)
                {
                    hash["id"] = model.ProductInfoId;
                    if(returnData){
                        ProductInfo newrow = dal.GetModelByID(model.ProductInfoId);
                        hash["data"] = newrow;
                    }
                }
            }
        ...
        [NonAction]        
        public  virtual List<ProductInfo> GetGridList(string filterId, int showValue, out int totalRows)
        {
            totalRows = 0;
            ...
            List<ProductInfo> modelList = new List<ProductInfo>();
            switch(filterId)
            {
                    case "ProductInfo_Models":
                    {
        ...
        public  virtual ActionResult GridByFilterID(string filterId)
        {
            ...
            catch (Exception ex)
            {
                hash["success"] = false;
                hash["message"] = ex.Message;
                hash["rows"] = new List<ProductInfo>();
            }
            return Content(JsonConvert.SerializeObject(hash)); 
        }
    ...
    public virtual ActionResult Delete(string id)
    {
        ...
        [CheckPurview(pageCode:"Product_ProductInfo_Show")]        
        public ActionResult GetEntityByID(string id)
        {
            ProductInfo model = dal.GetModelByID(id);
            if (model != null)
            {
                hash["data"] = model;
                hash["success"] = true;

...
//     如果想修改此文件的方法，请在此文件的上级目录中的VProductBookDetailController重写(override)该方法。
...
namespace Yamon.Module.Product.WebApi
{
    ...
    /// 预约信息视图
    /// </summary>
    public partial class _VProductBookDetailController :BaseController
    {

        public VProductBookDetailDAL dal = new VProductBookDetailDAL();
        ...
        [ValidateInput(false)]
        [LogFilter]
        public virtual ActionResult Create(VProductBookDetail model,bool returnData=false)
        {
            object newId = "";
            try
            {
                ...
                if (result > 0)
                {
                            newId = model.ProductBookDetailID;

                    hash["id"] = newId;
                    if(returnData){
                        VProductBookDetail newrow = dal.GetModelByID(newId);
                        hash["data"] = newrow;
                    }
                }
        ...
        [LogFilter]
        public virtual ActionResult Edit(VProductBookDetail model,bool returnData=false)
        {
            try
            {
               ...
               if (result > 0)
                {
                    hash["id"] = model.ProductBookDetailID;
                    if(returnData){
                        VProductBookDetail newrow = dal.GetModelByID(model.ProductBookDetailID);
                        hash["data"] = newrow;
                    }
                }
            }
        ...
        [NonAction]        
        public  virtual List<VProductBookDetail> GetGridList(string filterId, int showValue, out int totalRows)
        {
            totalRows = 0;
            ...
            List<VProductBookDetail> modelList = new List<VProductBookDetail>();
            switch(filterId)
            {
        ...
        public  virtual ActionResult GridByFilterID(string filterId)
        {
            ...
            catch (Exception ex)
            {
                hash["success"] = false;
                hash["message"] = ex.Message;
                hash["rows"] = new List<VProductBookDetail>();
            }
            return Content(JsonConvert.SerializeObject(hash)); 
        }
    ...
    public virtual ActionResult Delete(string id)
    {
        ...
        [CheckPurview(pageCode:"Product_VProductBookDetail_Show")]        
        public ActionResult GetEntityByID(string id)
        {
            VProductBookDetail model = dal.GetModelByID(id);
            if (model != null)
            {
                hash["data"] = model;
                hash["success"] = true;

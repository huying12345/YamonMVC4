...
//     如果想修改此文件的方法，请在此文件的上级目录中的ProductBookController重写(override)该方法。
...
namespace Yamon.Module.Product.WebApi
{
    ...
    /// 预约
    /// </summary>
    public partial class _ProductBookController :BaseController
    {

        public ProductBookDAL dal = new ProductBookDAL();
        ...
        [ValidateInput(false)]
        [LogFilter]
        public virtual ActionResult Create(ProductBook model,bool returnData=false)
        {
            object newId = "";
            try
            {
                ...
                if (result > 0)
                {
                            newId = model.ProductBookID;

                    hash["id"] = newId;
                    if(returnData){
                        ProductBook newrow = dal.GetModelByID(newId);
                        hash["data"] = newrow;
                    }
                }
        ...
        [LogFilter]
        public virtual ActionResult Edit(ProductBook model,bool returnData=false)
        {
            try
            {
               ...
               if (result > 0)
                {
                    hash["id"] = model.ProductBookID;
                    if(returnData){
                        ProductBook newrow = dal.GetModelByID(model.ProductBookID);
                        hash["data"] = newrow;
                    }
                }
            }
        ...
        [NonAction]        
        public  virtual List<ProductBook> GetGridList(string filterId, int showValue, out int totalRows)
        {
            totalRows = 0;
            ...
            List<ProductBook> modelList = new List<ProductBook>();
            switch(filterId)
            {
        ...
        public  virtual ActionResult GridByFilterID(string filterId)
        {
            ...
            catch (Exception ex)
            {
                hash["success"] = false;
                hash["message"] = ex.Message;
                hash["rows"] = new List<ProductBook>();
            }
            return Content(JsonConvert.SerializeObject(hash)); 
        }
    ...
    public virtual ActionResult Delete(string id)
    {
        ...
        [CheckPurview(pageCode:"Product_ProductBook_Show")]        

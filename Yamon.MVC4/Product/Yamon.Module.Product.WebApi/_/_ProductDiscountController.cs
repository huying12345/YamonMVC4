...
//     如果想修改此文件的方法，请在此文件的上级目录中的ProductDiscountController重写(override)该方法。
...
namespace Yamon.Module.Product.WebApi
{
    ...
    /// 商品折扣
    /// </summary>
    public partial class _ProductDiscountController :BaseController
    {

        public ProductDiscountDAL dal = new ProductDiscountDAL();
        ...
        [ValidateInput(false)]
        [LogFilter]
        public virtual ActionResult Create(ProductDiscount model,bool returnData=false)
        {
            object newId = "";
            try
            {
                ...
                if (result > 0)
                {
                    ...
                    hash["id"] = newId;
                    if(returnData){
                        ProductDiscount newrow = dal.GetModelByID(newId);
                        hash["data"] = newrow;
                    }
                }
        ...
        [LogFilter]
        public virtual ActionResult Edit(ProductDiscount model,bool returnData=false)
        {
            try
            {
               ...
               if (result > 0)
                {
                    hash["id"] = model.DiscountID;
                    if(returnData){
                        ProductDiscount newrow = dal.GetModelByID(model.DiscountID);
                        hash["data"] = newrow;
                    }
                }
            }
        ...
        [NonAction]        
        public  virtual List<ProductDiscount> GetGridList(string filterId, int showValue, out int totalRows)
        {
            totalRows = 0;
            ...
            List<ProductDiscount> modelList = new List<ProductDiscount>();
            switch(filterId)
            {
                    case "ProductDiscount_Models":
                    {
        ...
        {
            filterId = string.IsNullOrEmpty(filterId)?"ProductDiscount_Models":filterId;
            try
            {
                int totalRows = 0;
            ...
            catch (Exception ex)
            {
                hash["success"] = false;
                hash["message"] = ex.Message;
                hash["rows"] = new List<ProductDiscount>();
            }
            return Content(JsonConvert.SerializeObject(hash)); 
        }
    ...
    public virtual ActionResult Delete(string id)
    {
        ...
        [CheckPurview(pageCode:"Product_ProductDiscount_Show")]        
        public ActionResult GetEntityByID(string id)
        {
            ProductDiscount model = dal.GetModelByID(id);
            if (model != null)
            {
                hash["data"] = model;
                hash["success"] = true;

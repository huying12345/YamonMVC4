...
//     如果想修改此文件的方法，请在此文件的上级目录中的ProductCardController重写(override)该方法。
...
namespace Yamon.Module.Product.WebApi
{
    ...
    ///  体验卡
    /// </summary>
    public partial class _ProductCardController :BaseController
    {

        public ProductCardDAL dal = new ProductCardDAL();
        ...
        [ValidateInput(false)]
        [LogFilter]
        public virtual ActionResult Create(ProductCard model,bool returnData=false)
        {
            object newId = "";
            try
            {
                ...
                if (result > 0)
                {
                    ...
                    hash["id"] = newId;
                    if(returnData){
                        ProductCard newrow = dal.GetModelByID(newId);
                        hash["data"] = newrow;
                    }
                }
        ...
        [LogFilter]
        public virtual ActionResult Edit(ProductCard model,bool returnData=false)
        {
            try
            {
               ...
               if (result > 0)
                {
                    hash["id"] = model.CardID;
                    if(returnData){
                        ProductCard newrow = dal.GetModelByID(model.CardID);
                        hash["data"] = newrow;
                    }
                }
            }
        ...
        [NonAction]        
        public  virtual List<ProductCard> GetGridList(string filterId, int showValue, out int totalRows)
        {
            totalRows = 0;
            ...
            List<ProductCard> modelList = new List<ProductCard>();
            switch(filterId)
            {
        ...
        public  virtual ActionResult GridByFilterID(string filterId)
        {
            ...
            catch (Exception ex)
            {
                hash["success"] = false;
                hash["message"] = ex.Message;
                hash["rows"] = new List<ProductCard>();
            }
            return Content(JsonConvert.SerializeObject(hash)); 
        }
    ...
    public virtual ActionResult Delete(string id)
    {
        ...
        [CheckPurview(pageCode:"Product_ProductCard_Show")]        
        public ActionResult GetEntityByID(string id)
        {
            ProductCard model = dal.GetModelByID(id);
            if (model != null)
            {
                hash["data"] = model;
                hash["success"] = true;

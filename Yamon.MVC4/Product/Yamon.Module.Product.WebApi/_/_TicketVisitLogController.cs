...
//     如果想修改此文件的方法，请在此文件的上级目录中的TicketVisitLogController重写(override)该方法。
...
namespace Yamon.Module.Product.WebApi
{
    ...
    /// 验票记录表
    /// </summary>
    public partial class _TicketVisitLogController :BaseController
    {

        public TicketVisitLogDAL dal = new TicketVisitLogDAL();
        ...
        [ValidateInput(false)]
        [LogFilter]
        public virtual ActionResult Create(TicketVisitLog model,bool returnData=false)
        {
            object newId = "";
            try
            {
                ...
                if (result > 0)
                {
                    ...
                    hash["id"] = newId;
                    if(returnData){
                        TicketVisitLog newrow = dal.GetModelByID(newId);
                        hash["data"] = newrow;
                    }
                }
        ...
        [LogFilter]
        public virtual ActionResult Edit(TicketVisitLog model,bool returnData=false)
        {
            try
            {
               ...
               if (result > 0)
                {
                    hash["id"] = model.LogID;
                    if(returnData){
                        TicketVisitLog newrow = dal.GetModelByID(model.LogID);
                        hash["data"] = newrow;
                    }
                }
            }
        ...
        [NonAction]        
        public  virtual List<TicketVisitLog> GetGridList(string filterId, int showValue, out int totalRows)
        {
            totalRows = 0;
            ...
            List<TicketVisitLog> modelList = new List<TicketVisitLog>();
            switch(filterId)
            {
        ...
        public  virtual ActionResult GridByFilterID(string filterId)
        {
            ...
            catch (Exception ex)
            {
                hash["success"] = false;
                hash["message"] = ex.Message;
                hash["rows"] = new List<TicketVisitLog>();
            }
            return Content(JsonConvert.SerializeObject(hash)); 
        }
    ...
    public virtual ActionResult Delete(string id)
    {
        ...
        [CheckPurview(pageCode:"Product_TicketVisitLog_Show")]        
        public ActionResult GetEntityByID(string id)
        {
            TicketVisitLog model = dal.GetModelByID(id);
            if (model != null)
            {
                hash["data"] = model;
                hash["success"] = true;

...
//     如果想修改此文件的方法，请在此文件的上级目录中的ProductCardRecordController重写(override)该方法。
...
namespace Yamon.Module.Product.WebApi
{
    ...
    /// 体验卡交易记录
    /// </summary>
    public partial class _ProductCardRecordController :BaseController
    {

        public ProductCardRecordDAL dal = new ProductCardRecordDAL();

        public virtual ActionResult Grid1_CardID()
        {
             return GridByFilterID("ProductCardRecord_CardID");
        } 
        ...
        [ValidateInput(false)]
        [LogFilter]
        public virtual ActionResult Create(ProductCardRecord model,bool returnData=false)
        {
            object newId = "";
            try
            {
                ...
                if (result > 0)
                {
                    ...
                    hash["id"] = newId;
                    if(returnData){
                        ProductCardRecord newrow = dal.GetModelByID(newId);
                        hash["data"] = newrow;
                    }
                }
        ...
        [LogFilter]
        public virtual ActionResult Edit(ProductCardRecord model,bool returnData=false)
        {
            try
            {
               ...
               if (result > 0)
                {
                    hash["id"] = model.RecordID;
                    if(returnData){
                        ProductCardRecord newrow = dal.GetModelByID(model.RecordID);
                        hash["data"] = newrow;
                    }
                }
            }
        ...
        [NonAction]        
        public  virtual List<ProductCardRecord> GetGridList(string filterId, int showValue, out int totalRows)
        {
            totalRows = 0;
            ...
            List<ProductCardRecord> modelList = new List<ProductCardRecord>();
            switch(filterId)
            {
                    case "ProductCardRecord_CardID":
                    {
                      modelList = dal.GetEntityListByPage_CardID(out totalRows,0,page,rows,orderby);
                      break;
                    }
        ...
        public  virtual ActionResult GridByFilterID(string filterId)
        {
            ...
            catch (Exception ex)
            {
                hash["success"] = false;
                hash["message"] = ex.Message;
                hash["rows"] = new List<ProductCardRecord>();
            }
            return Content(JsonConvert.SerializeObject(hash)); 
        }
    ...
    public virtual ActionResult Delete(string id)
    {
        ...
        [CheckPurview(pageCode:"Product_ProductCardRecord_Show")]        
        public ActionResult GetEntityByID(string id)
        {
            ProductCardRecord model = dal.GetModelByID(id);
            if (model != null)
            {
                hash["data"] = model;
                hash["success"] = true;

...
//     如果想修改此文件的方法，请在此文件的上级目录中的OrderDetailController重写(override)该方法。
...
namespace Yamon.Module.Product.WebApi
{
    ...
    /// 订单明细
    /// </summary>
    public partial class _OrderDetailController :BaseController
    {

        public OrderDetailDAL dal = new OrderDetailDAL();
        ...
        [ValidateInput(false)]
        [LogFilter]
        public virtual ActionResult Create(OrderDetail model,bool returnData=false)
        {
            object newId = "";
            try
            {
                ...
                if (result > 0)
                {
                            newId = model.OrderDetailID;

                    hash["id"] = newId;
                    if(returnData){
                        OrderDetail newrow = dal.GetModelByID(newId);
                        hash["data"] = newrow;
                    }
                }
        ...
        [LogFilter]
        public virtual ActionResult Edit(OrderDetail model,bool returnData=false)
        {
            try
            {
               ...
               if (result > 0)
                {
                    hash["id"] = model.OrderDetailID;
                    if(returnData){
                        OrderDetail newrow = dal.GetModelByID(model.OrderDetailID);
                        hash["data"] = newrow;
                    }
                }
            }
        ...
        [NonAction]        
        public  virtual List<OrderDetail> GetGridList(string filterId, int showValue, out int totalRows)
        {
            totalRows = 0;
        ...
        public  virtual ActionResult GridByFilterID(string filterId)
        {
            ...
            catch (Exception ex)
            {
                hash["success"] = false;
                hash["message"] = ex.Message;
                hash["rows"] = new List<OrderDetail>();
            }
            return Content(JsonConvert.SerializeObject(hash)); 
        }
    ...
    public virtual ActionResult Delete(string id)
    {
        ...
        public ActionResult ImportFromExcel()
        {
            ...
            return Json(hash);
        }        [CheckPurview(pageCode:"Product_OrderDetail_Show")]        
        public ActionResult GetEntityByID(string id)
        {
            OrderDetail model = dal.GetModelByID(id);
            if (model != null)
            {
                hash["data"] = model;
                hash["success"] = true;

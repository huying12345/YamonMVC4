...
//     如果想修改此文件的方法，请在此文件的上级目录中的ProductConfigController重写(override)该方法。
...
namespace Yamon.Module.Product.WebApi
{
    ...
    /// 商品模块配置
    /// </summary>
    public partial class _ProductConfigController :BaseController
    {

        public ProductConfigDAL dal = new ProductConfigDAL();

        

        [ValidateInput(false)]
        [LogFilter]
        public virtual ActionResult Edit(ProductConfig model,bool returnData=false)
        {
            try
            {
               ...
               if (result > 0)
                {
                    hash["id"] = model.ConfigKey;
                    if(returnData){
                        ProductConfig newrow = dal.GetModelByID(model.ConfigKey);
                        hash["data"] = newrow;
                    }
                }
            }
        ...
        [NonAction]        
        public  virtual List<ProductConfig> GetGridList(string filterId, int showValue, out int totalRows)
        {
            totalRows = 0;
            ...
            List<ProductConfig> modelList = new List<ProductConfig>();
            switch(filterId)
            {
        ...
        public  virtual ActionResult GridByFilterID(string filterId)
        {
            ...
            catch (Exception ex)
            {
                hash["success"] = false;
                hash["message"] = ex.Message;
                hash["rows"] = new List<ProductConfig>();
            }
            return Content(JsonConvert.SerializeObject(hash)); 
        }
    ...
    public virtual ActionResult Delete(string id)
    {
        ...
        [CheckPurview(pageCode:"Product_ProductConfig_Show")]        
        public ActionResult GetEntityByID(string id)
        {
            ProductConfig model = dal.GetModelByID(id);
            if (model != null)
            {
                hash["data"] = model;
                hash["success"] = true;

...
//     如果想修改此文件的方法，请在此文件的上级目录中的VOrderRefundController重写(override)该方法。
...
namespace Yamon.Module.Product.WebApi
{
    ...
    /// 退单视图
    /// </summary>
    public partial class _VOrderRefundController :BaseController
    {

        public VOrderRefundDAL dal = new VOrderRefundDAL();
        ...
        [NonAction]        
        public  virtual List<VOrderRefund> GetGridList(string filterId, int showValue, out int totalRows)
        {
            totalRows = 0;
            ...
            List<VOrderRefund> modelList = new List<VOrderRefund>();
            switch(filterId)
            {
                    case "VOrderRefund_Models":
                    {
        ...
        {
            filterId = string.IsNullOrEmpty(filterId)?"VOrderRefund_Models":filterId;
            try
            {
                int totalRows = 0;
            ...
            catch (Exception ex)
            {
                ...
                hash["rows"] = new List<VOrderRefund>();
            }
    ...
    public virtual ActionResult Delete(string id)
    {
        ...
        [CheckPurview(pageCode:"Product_VOrderRefund_Show")]        
        public ActionResult GetEntityByID(string id)
        {
            VOrderRefund model = dal.GetModelByID(id);
            if (model != null)
            {
                hash["data"] = model;
                hash["success"] = true;

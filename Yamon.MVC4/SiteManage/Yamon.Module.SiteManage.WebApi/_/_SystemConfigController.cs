...
//     如果想修改此文件的方法，请在此文件的上级目录中的SystemConfigController重写(override)该方法。
...
namespace Yamon.Module.SiteManage.WebApi
{
    ...
    /// 系统配置
    /// </summary>
    public partial class _SystemConfigController :BaseController
    {

        public SystemConfigDAL dal = new SystemConfigDAL();

        

        [ValidateInput(false)]
        [LogFilter]
        public virtual ActionResult Edit(SystemConfig model,bool returnData=false)
        {
            try
            {
               ...
               if (result > 0)
                {
                    ...
                    if(returnData){
                        SystemConfig newrow = dal.GetModelByID(model.ConfigKey);
                        hash["data"] = newrow;
                    }
                }
            }
        ...
        [NonAction]        
        public  virtual List<SystemConfig> GetGridList(string filterId, int showValue, out int totalRows)
        {
            totalRows = 0;
            ...
            List<SystemConfig> modelList = new List<SystemConfig>();
            switch(filterId)
            {
        ...
        public  virtual ActionResult GridByFilterID(string filterId)
        {
            ...
            catch (Exception ex)
            {
                hash["success"] = false;
                hash["message"] = ex.Message;
                hash["rows"] = new List<SystemConfig>();
            }
            return Content(JsonConvert.SerializeObject(hash)); 
        }
    ...
    public virtual ActionResult Delete(string id)
    {
        ...
        [CheckPurview(pageCode:"SiteManage_SystemConfig_Show")]        
        public ActionResult GetEntityByID(string id)
        {
            SystemConfig model = dal.GetModelByID(id);
            if (model != null)
            {
                hash["data"] = model;
                hash["success"] = true;

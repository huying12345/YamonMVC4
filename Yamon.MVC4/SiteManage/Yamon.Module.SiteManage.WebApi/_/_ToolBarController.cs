...
//     如果想修改此文件的方法，请在此文件的上级目录中的ToolBarController重写(override)该方法。
...
namespace Yamon.Module.SiteManage.WebApi
{
    ...
    /// 工具栏模型
    /// </summary>
    public partial class _ToolBarController :BaseController
    {

        public ToolBarDAL dal = new ToolBarDAL();
        ...
        [ValidateInput(false)]
        [LogFilter]
        public virtual ActionResult Create(ToolBar model,bool returnData=false)
        {
            object newId = "";
            try
            {
                ...
                if (result > 0)
                {
                            newId = model.ToolBarID;

                    hash["id"] = newId;
                    if(returnData){
                        ToolBar newrow = dal.GetModelByID(newId);
                        hash["data"] = newrow;
                    }
                }
        ...
        [LogFilter]
        public virtual ActionResult Edit(ToolBar model,bool returnData=false)
        {
            try
            {
               ...
               if (result > 0)
                {
                    hash["id"] = model.ToolBarID;
                    if(returnData){
                        ToolBar newrow = dal.GetModelByID(model.ToolBarID);
                        hash["data"] = newrow;
                    }
                }
            }
        ...
        [NonAction]        
        public  virtual List<ToolBar> GetGridList(string filterId, int showValue, out int totalRows)
        {
            totalRows = 0;
            ...
            List<ToolBar> modelList = new List<ToolBar>();
            switch(filterId)
            {
                    case "ToolBar_MenuID":
                    {
                      modelList = dal.GetEntityListByPage_MenuID(out totalRows,0,page,rows,orderby);
                      break;
                    }
        ...
        {
            filterId = string.IsNullOrEmpty(filterId)?"ToolBar_MenuID":filterId;
            try
            {
                int totalRows = 0;
    ...
    public virtual ActionResult Delete(string id)
    {
        ...
        [CheckPurview(pageCode:"SiteManage_ToolBar_Show")]        
        public ActionResult GetEntityByID(string id)
        {
            ToolBar model = dal.GetModelByID(id);
            if (model != null)
            {
                hash["data"] = model;
                hash["success"] = true;

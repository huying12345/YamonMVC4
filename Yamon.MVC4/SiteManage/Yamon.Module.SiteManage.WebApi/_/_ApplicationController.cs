...
//     如果想修改此文件的方法，请在此文件的上级目录中的ApplicationController重写(override)该方法。
...
namespace Yamon.Module.SiteManage.WebApi
{
    ...
    /// 应用
    /// </summary>
    public partial class _ApplicationController :BaseController
    {

        public ApplicationDAL dal = new ApplicationDAL();
        ...
        [ValidateInput(false)]
        [LogFilter]
        public virtual ActionResult Create(Application model,bool returnData=false)
        {
            object newId = "";
            try
            {
                ...
                if (result > 0)
                {
                    ...
                    hash["id"] = newId;
                    if(returnData){
                        Application newrow = dal.GetModelByID(newId);
                        hash["data"] = newrow;
                    }
                }
        ...
        [LogFilter]
        public virtual ActionResult Edit(Application model,bool returnData=false)
        {
            try
            {
               ...
               if (result > 0)
                {
                    hash["id"] = model.AppID;
                    if(returnData){
                        Application newrow = dal.GetModelByID(model.AppID);
                        hash["data"] = newrow;
                    }
                }
            }
        ...
        [NonAction]        
        public  virtual List<Application> GetGridList(string filterId, int showValue, out int totalRows)
        {
            totalRows = 0;
            ...
            List<Application> modelList = new List<Application>();
            switch(filterId)
            {
        ...
        public  virtual ActionResult GridByFilterID(string filterId)
        {
            ...
            catch (Exception ex)
            {
                hash["success"] = false;
                hash["message"] = ex.Message;
                hash["rows"] = new List<Application>();
            }
            return Content(JsonConvert.SerializeObject(hash)); 
        }
    ...
    public virtual ActionResult Delete(string id)
    {
        ...
        [CheckPurview(pageCode:"SiteManage_Application_Show")]        
        public ActionResult GetEntityByID(string id)
        {
            Application model = dal.GetModelByID(id);
            if (model != null)
            {
                hash["data"] = model;
                hash["success"] = true;

...
//     如果想修改此文件的方法，请在此文件的上级目录中的SubPlanController重写(override)该方法。
...
namespace Yamon.Module.SiteManage.WebApi
{
    ...
    /// 计划任务模型
    /// </summary>
    public partial class _SubPlanController :BaseController
    {

        public SubPlanDAL dal = new SubPlanDAL();
        ...
        [ValidateInput(false)]
        [LogFilter]
        public virtual ActionResult Create(SubPlan model,bool returnData=false)
        {
            object newId = "";
            try
            {
                ...
                if (result > 0)
                {
                    ...
                    hash["id"] = newId;
                    if(returnData){
                        SubPlan newrow = dal.GetModelByID(newId);
                        hash["data"] = newrow;
                    }
                }
        ...
        [LogFilter]
        public virtual ActionResult Edit(SubPlan model,bool returnData=false)
        {
            try
            {
               ...
               if (result > 0)
                {
                    hash["id"] = model.SubPlanID;
                    if(returnData){
                        SubPlan newrow = dal.GetModelByID(model.SubPlanID);
                        hash["data"] = newrow;
                    }
                }
            }
        ...
        [NonAction]        
        public  virtual List<SubPlan> GetGridList(string filterId, int showValue, out int totalRows)
        {
            totalRows = 0;
            ...
            List<SubPlan> modelList = new List<SubPlan>();
            switch(filterId)
            {
    ...
    public virtual ActionResult Delete(string id)
    {
        ...
        [CheckPurview(pageCode:"SiteManage_SubPlan_Show")]        
        public ActionResult GetEntityByID(string id)
        {
            SubPlan model = dal.GetModelByID(id);
            if (model != null)
            {
                hash["data"] = model;
                hash["success"] = true;

...
//     如果想修改此文件的方法，请在此文件的上级目录中的DictDataController重写(override)该方法。
...
namespace Yamon.Module.SiteManage.WebApi
{
    ...
    /// 数据字典
    /// </summary>
    public partial class _DictDataController :BaseController
    {

        public DictDataDAL dal = new DictDataDAL();
        ...
        [ValidateInput(false)]
        [LogFilter]
        public virtual ActionResult Create(DictData model,bool returnData=false)
        {
            object newId = "";
            try
            {
                ...
                if (result > 0)
                {
                    ...
                    hash["id"] = newId;
                    if(returnData){
                        DictData newrow = dal.GetModelByID(newId);
                        hash["data"] = newrow;
                    }
                }
        ...
        [LogFilter]
        public virtual ActionResult Edit(DictData model,bool returnData=false)
        {
            try
            {
               ...
               if (result > 0)
                {
                    hash["id"] = model.ID;
                    if(returnData){
                        DictData newrow = dal.GetModelByID(model.ID);
                        hash["data"] = newrow;
                    }
                }
            }
        ...
        [NonAction]        
        public  virtual List<DictData> GetGridList(string filterId, int showValue, out int totalRows)
        {
            totalRows = 0;
            ...
            List<DictData> modelList = new List<DictData>();
            switch(filterId)
            {
                    case "DictData_DictTypeID":
                    {
                      modelList = dal.GetEntityListByPage_DictTypeID(out totalRows,0,page,rows,orderby);
                      break;
                    }
        ...
        {
            filterId = string.IsNullOrEmpty(filterId)?"DictData_DictTypeID":filterId;
            try
            {
                int totalRows = 0;
    ...
    public virtual ActionResult Delete(string id)
    {
        ...
        [CheckPurview(pageCode:"SiteManage_DictData_Show")]        
        public ActionResult GetEntityByID(string id)
        {
            DictData model = dal.GetModelByID(id);
            if (model != null)
            {
                hash["data"] = model;
                hash["success"] = true;

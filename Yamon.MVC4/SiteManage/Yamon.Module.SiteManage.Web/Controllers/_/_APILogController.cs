...
//     如果想修改此文件的方法，请在此文件的上级目录中的APILogController重写(override)该方法。
...
namespace Yamon.Module.SiteManage.Web.Controllers
{
    ...
    /// 接口日志
    /// </summary>
    public partial class _APILogController :BaseController
    {

        public APILogDAL dal = new APILogDAL();
        ...
        public ActionResult Index(string m,string filterId)
        {
            ...
            string formPath = string.Format("~/Areas/SiteManage/Views/APILog/_/{0}.cshtml", m);
...
public virtual ActionResult Create(string id = "")
{
    ViewBag.ID = id;
    ViewBag.DAL=dal;
    APILog model= dal.GetCreateFormDefaultValue();
    model=dal.GetModelShowValue(model);
    string formPath = SiteCommon.GetCustomPage("~/Areas/SiteManage/Views/APILog/_/Create.cshtml");
...
public virtual ActionResult Show(string id = "")
{
    ViewBag.ID = id;
    ViewBag.DAL=dal;
    APILog model = dal.GetModelByID(id);
    model=dal.GetModelShowValue(model);
    string formPath = SiteCommon.GetCustomPage("~/Areas/SiteManage/Views/APILog/_/Show.cshtml");
    return View(formPath,model);
}
